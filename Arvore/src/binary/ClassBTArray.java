...
public class ClassBTArray{
    ...
    public Object remove(NodeBT no, int key) throws InvalidPositionException {
        ...
        if (isExternal(no)){
            //comparo se o nó a ser removido está do lado direito ou esquerdo do pai
            //nesse caso lado direito
            if( no.getKey() == no.getParent().getRight().getKey()){
                aux = no.getElement();
                no.getParent().setRight(null);
            }
            //nesse caso lado esquerdo
            else{
                aux = no.getElement();
